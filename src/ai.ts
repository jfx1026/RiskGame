...
import { GameState, getCurrentTeam } from './game.js';
...
export type AIPersonality =
    | 'aggressive'   // Pink: Attacks often, even with bad odds
    | 'strategic'    // Coral: Calculated, only attacks with good odds
    | 'positional'   // Gold: Prioritizes key positions, territories with many neighbors
    | 'longterm'     // Emerald: Conservative, builds up before attacking
    | 'connector'    // Ocean: Focuses on connecting territories for reinforcement bonus
    | 'erratic';     // Bronze: Random strategy each turn

/**
 * Map team IDs to their personalities
 */
const TEAM_PERSONALITIES: AIPersonality[] = [
    'aggressive',   // 0: Pink
    'strategic',    // 1: Coral
    'positional',   // 2: Gold
    'longterm',     // 3: Emerald
 ...
 */
export function getTeamPersonality(teamId: number): AIPersonality {
...
function calculateAttackScore(
    ...
    switch (personality) {
        ...
        case 'longterm':
            ...
            if (ratio < 1.2) score -= 15;  // Strong penalty for risky attacks
            if (source.armies >= 5) score += 8;  // Bonus when attacking from strength
            // Prefer to keep armies for defense
            if (source.armies <= 3) score -= 10;
            break;
 ...
 */
function countEnemyNeighbors(target: Territory, territories: Territory[], teamId: number): number {
    let count = 0;
 ...
 */
function wouldConnectTerritories(target: Territory, territories: Territory[], teamId: number): boolean {
    // Count how many of our territory groups this target neighbors
    const ourNeighboringTerritories: number[] = [];
    ...
    if (ourNeighboringTerritories.length < 2) return false;
